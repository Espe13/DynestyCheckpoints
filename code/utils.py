...
def print_fn_tqdm(pbar,
                  ...
                  logl_max=np.inf):
    """
    This function prints the status using a custom write function to mimic
    the tqdm style, with formatted output similar to the desired format.
    """
    # Extract results at the current iteration.
    loglstar = results.loglstar
    logz = results.logz
    ...
    if dlogz is not None:
        status_string = ("\riter: {:d} | batch: {:d} | nc: {:d} | "
                        "ncall: {:d} | eff(%): {:6.3f} | "
                        "logz: {:6.3f} +/- {:6.3f} | "
                        "dlogz: {:6.3f} > {:6.3f} \n"
                        .format(niter, nbatch if nbatch is not None else 0, nc, ncall, eff, logz,
                                logzerr, delta_logz, dlogz))
    else:
        status_string = ("\riter: {:d} | batch: {:d} | "
                         "nc: {:d} | ncall: {:d} | "
                         "eff(%): {:6.3f} | "
                         "loglstar: {:6.3f} < {:6.3f} "
                         "< {:6.3f} | "
                         "logz: {:6.3f} +/- {:6.3f} | "
                         "stop: {:6.3f} \n"
                        .format(niter, nbatch + 1 if nbatch is not None else 0, nc, ncall, eff, logl_min, loglstar, logl_max, logz, logzerr, stop_val))

    # Print the status string to the console
    print(status_string, end='')

    # Update the tqdm progress bar
    pbar.update(niter - pbar.n)
    
def print_fn_tqdm_old(pbar,
                  results,
                  niter,
                  ncall,
                  add_live_it=None,
